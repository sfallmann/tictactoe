...
function findTile(x, y, tiles){
  var tileFilter = this.tiles.filter(function(tile){
...
function get_columns(numCols, numRows, tiles) {
  var cols = [];

  for (var x = 0; x < numRows; x++){
    var col = [];
    for (var y = 0; y < numCols; y++) {
      col.push(findTile(x, y, tiles).value);
    }
    cols.push(col);
  }
  return cols;
...
function get_rows(numCol, numRow, tiles) {
  var rows = [];

  for (var y = 0; y < numCols; y++){
    var row = [];
    for (var x = 0; x < numRows; x++) {
      row.push(findTile(x, y).value);
    }
    rows.push(row);
  }
  return rows;
...
var TicTacToe = (function() {
  ...
  function chooseTile() {
    ...
    function over(tiles) {
      var rows = get_rows(3, 3, tiles);
      var cols = get_columns(3, 3, tiles);
    ...
    function minimax(game, depth) {
      ...
      game.board.emptyTiles(game.board.tiles()).forEach(function(tile){
